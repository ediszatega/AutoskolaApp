...
namespace Autoskola.API.Controllers
{
    ...
    public class InstructorController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Add([FromBody] InstructorAddVM instructor)
        {
            var result = await service.Add(instructor);
            return Ok(result);
        }
        [HttpPut]
        public async Task<IActionResult> Update([FromBody] InstructorUpdateVM instructor)
        {
            var result = await service.Update(instructor);
            return Ok(result);
        }
        [HttpDelete("{id}")]
