...
namespace Autoskola.Repository.Repositories
{
    public class InstructorRepository : Repository<Instructor>, IInstructorRepository
    {
        ...
        public async Task<IEnumerable<Instructor>> GetAllIncludeCities(string? search, int pageNumber = 1, int pageSize = 100)
        {
            try
            {

                return await AutoskolaContext.Instructors.Where(user => string.IsNullOrEmpty(search) ||
