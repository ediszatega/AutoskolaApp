...
namespace Autoskola.Repository.Repositories
{
    public class CustomerRepository : Repository<Customer>, ICustomerRepository 
    {
        ...
        public async Task<IEnumerable<Customer>> GetAllIncludeCities(string? search, int pageNumber = 1, int pageSize = 100)
        {
            try
            {

                return await AutoskolaContext.Customers.Where(user => string.IsNullOrEmpty(search) ||
